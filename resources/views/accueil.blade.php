...
<div class="button" >
<button type="button" class="btn btn-primary"><a href="{{route('VoirPlus')}}" style="text-decoration: none; color:white;">Ajouter un étudiant</a></button>
</div>
<table class="table table-striped">
  ...
  <tbody>
        @foreach ($etudiant_list as $item)
          <tr>
            <th scope="row"><img src="{{asset($item['images'])}}" style=" border-radius:50%; height:50px; width:50px;" alt="" ></th>
            <td> {{$item['name']}} </td>
            <td>{{$item['hobbies']}} </td>
            <td>{{$item['spécialité']}} </td>
            <td>
              @if($item->active)
                      <button class="btn btn-danger"><a href="{{ route('deactivate', ['id' => $item->id]) }}" style="text-decoration: none; color:white;">Désactiver</a></button>
              @else
                      <button class="btn btn-success"><a href="{{ route('activate', ['id' => $item->id]) }}" style="text-decoration: none; color:white;">Activer</a></button>
              @endif
          </td>
            <td>
